...
open class ObjectDecoder(
    ...
    override fun ObjectDefinition.read(opcode: Int, buffer: Reader) {
        when (opcode) {
            ...
            186 -> buffer.skip(1) // bgsoundshape
            188 -> { } // boolean flag
            189 -> dynamicTint = true
            in 190..195 -> buffer.readUnsignedShort() // actionCursors
            196 -> buffer.skip(1) // unknown byte
            197 -> buffer.skip(1) // unknown byte
            198, 199, 200, 203 -> { } // boolean flags
            201 -> repeat(6) { buffer.readSmart() } // signedSmart array
            202 -> buffer.readSmart() // unsignedSmart
            204 -> {
                // Complex structure skip
                val count = buffer.readSmart()
                for (i in 0 until count) {
                    buffer.readUnsignedShort() // ushort
                    buffer.skip(1) // byte (boolean)
                    buffer.skip(24) // 6 ints
                }
            }
            205 -> {
            ...
            250 -> buffer.skip(1) // bgsoundshape
            251 -> buffer.skip(1) // bgsounddistancefiltered
            252 -> buffer.skip(6) // 3 shorts (hzhigh, hzlow, q)
            253 -> buffer.skip(1) // randomsoundshape
            254 -> buffer.skip(1) // randomsounddistancefiltered
            255 -> buffer.skip(6) // 3 shorts (randomsounddistancefilterparams)
            else -> { }
        }
    }
