...
internal fun Codec.registerRev948ClientProtStubs() {
    ...
    c(8, "UNKNOWN_8", 4)                              // CONF:NONE — fn SendMultiDisplayPackets @ 0x001a2d90 (entry 0x015d3e20); no MULTI_DISPLAY official enum. 948-5 sender attribution: jag::ClientWatch::MainLogic — still no official jag::ClientProt enum match; stays UNKNOWN_8 (see docs/net/948-5-delta-from-948-2.md §3, Q5 periodic-emit family).
    c(9, "SEND_NATIVE_MOUSE_CLICK", 12)               // CONF:MEDIUM — 948-5 attributes the descriptor to jag::ClientProt::SendNativeMouseClick (sender symbol; not an official jag::ClientProt::<NAME> enum). Supersedes the prior 948-2 attribution to FUN_0015b5c0 "use-item-on-loc", which was mis-walked (op 6's SendOpLocU was double-counted onto op 9). 948-5 schema (totalBytes=12 lengthMatched): @0 shortLE, @2 intLE (guessed), @6 shortLE, @8 shortLE, @10 shortLEA — consistent with a native-mouse-event telemetry record (NOT use-item-on-loc). No decoder registered; layout still needs a deliberate Ghidra walk of SendNativeMouseClick on the live 948-2-2 binary before wiring (see docs/net/948-5-delta-from-948-2.md §3 op 9, Q2).
    ...
    c(51, "NO_TIMEOUT", 0)                            // CONF:CERTAIN — the keepalive. beta ConnectionManager::MainLogic→&ClientProt::NO_TIMEOUT on 0x32-tick counter; 948 ProcessConnections @ 0x0013e6c0 byte-identical port (entry 0x015d3b80). Client floods this every ~1s. 948-5 cross-confirm: jag::ConnectionManager::MainLogic recorded as the descriptor's owning sender (×2 emitters; see docs/net/948-5-delta-from-948-2.md §3 op 51).
    c(52, "UNKNOWN_52", 6)                            // CONF:NONE — 948-2 emitter SendDisplayInfo @ 0x001a2ba0 (entry 0x015d3b70); no DISPLAY_INFO official enum. 948-5 attributes the descriptor to jag::ClientWatch::SendWindowStatus, BUT op 94 already holds the official WINDOW_STATUS (size 3) — op 52 (size 6) is a SISTER packet (likely a longer display-info / window-event variant) with no distinct official jag::ClientProt enum. Stays UNKNOWN_52. TODO(948-5 Q3): op 52 vs op 94 WINDOW_STATUS ambiguity — see docs/net/948-5-delta-from-948-2.md
    ...
    c(81, "UNKNOWN_81", -2)                           // CONF:NONE — generic SendMultiDisplayPackets PARAM ref only. 948-5 sender attribution: jag::ClientWatch::MainLogic — periodic-emit family (same as ops 8/12/106). Still no official jag::ClientProt enum match; stays UNKNOWN_81 (see docs/net/948-5-delta-from-948-2.md §3 op 81, Q5 periodic-emit family).
    ...
    c(106, "UNKNOWN_106", 1)                          // CONF:NONE — fn SendMultiDisplayPackets @ 0x001a2d90 (entry 0x015d3810); no MULTI_DISPLAY official enum. 948-5 sender attribution: jag::ClientWatch::MainLogic — periodic-emit family (same as ops 8/12/81). Still no official jag::ClientProt enum match; stays UNKNOWN_106 (see docs/net/948-5-delta-from-948-2.md §3 op 106, Q5 periodic-emit family).
