...
**⚠️ NEVER reference the 216 binary for protocol.** The Ghidra project contains `osclient-216-mac` (an **OSRS debug build**). OSRS and RS3 are **separate games with incompatible, separately-numbered network protocols** — OSRS has far fewer opcodes. **Do not take any opcode, packet format, ClientProt/ServerProt entry, handler, or protocol sequence from 216** — it will desync the RS3 948 client. ALL protocol comes from the 948-5 binary + 948-5 production captures only. (216 is an *engine-level* naming reference for the ghidra agent, never a protocol source — see `docs/binary/216-osrs-nxt-reference.md`.)
