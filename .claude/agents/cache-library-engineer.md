...
**⚠️ The 216 binary is an OSRS debug build — engine-reference only.** Its embedded NXT *engine* (~935) shares cache/buffer plumbing with 948 and can help name stripped 948 functions, but **OSRS and RS3 are separate games** — never assume 216's cache *formats*, definition layouts, compression, or archive/index numbering match RS3 948; verify against the 948-5 binary + 948-5 cache. 948-5 is the only truth. See `docs/binary/216-osrs-nxt-reference.md`.
