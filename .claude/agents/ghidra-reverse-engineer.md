...
**THE 216 BINARY IS OSRS — ENGINE-REFERENCE ONLY, NEVER PROTOCOL.** The `rs2client-948` project also holds `osclient-216-mac`, a symbol-rich **OSRS (Old School) DEBUG build**. OSRS and RS3 are **separate games**: its opcodes / protocol / packet-formats / handlers (`jag::oldscape::*`) are OSRS-specific and **must NOT be ported to RS3 948** (OSRS has far fewer, incompatible opcodes). 216 is useful ONLY because it embeds an **NXT engine lib (~935** — same engine family as 948), so use its symbols STRICTLY to NAME 948's stripped *engine* functions (rendering/scene/map/cache/camera/`jag::Packet`), verifying ~935→948 drift against 948. **Debug-vs-release INLINING (critical):** 216 (debug) has small helpers (buffer `gT*`/`pT*`, accessors, short methods) as **separate named functions**; 948 (release) **INLINES** them — so a named 216 helper usually maps to an **inlined pattern inside a larger 948 function, NOT a 1:1 function** (and some 216 fns have no standalone 948 counterpart). Use 216 to learn an operation's name/signature/semantics, then recognize the inlined pattern in the 948 decompile; never expect matching function boundaries/call graphs. Target `rs2client.948-5` (base `0x0`) is the ONLY truth; 216 is base `0x100000000`. Switch the MCP's active program via a `run_java_class` script calling `ProgramManager.openProgram`. Full policy: `docs/binary/216-osrs-nxt-reference.md`.
