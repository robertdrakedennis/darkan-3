...
import com.undercut.util.getDirDelta
...
enum class Direction(@JvmField val id: Int, @JvmField val dx: Int, @JvmField val dy: Int) {
    ...
    companion object {
        @JvmStatic fun random() = entries.random()
        ...
        fun fromAngle(angle: Int): Direction? {
            val delta = getDirDelta(angle) ?: return SOUTH
            return forDelta(delta.first, delta.second)
        }
        ...
        fun forDelta(dx: Int, dy: Int): Direction? {
            ...
            else if (dx >= 1) EAST else if (dy <= -1) SOUTH else if (dx <= -1) WEST else null
        }

        @JvmStatic
        fun getAngleTo(dir: Direction): Int {
            return ((atan2(-dir.dx.toDouble(), -dir.dy.toDouble()) * 2607.5945876176133)
                    .toInt()) and 0x3fff
        }
