package com.undercut.script.impl.trent.necromancy
...
class NecromancyRituals : StateMachineScript<NecromancyRituals>() {
    override fun getStartState() = CheckDisturbances
}

object CheckDisturbances: State<NecromancyRituals>() {
    override suspend fun NecromancyRituals.checkNext() = if (localPlayer.animationId != 35520) StartRitual else checkDisturbanceState()

    override suspend fun NecromancyRituals.stateLoop() {
        delay(50, 150)
    }
}
...
private data class Disturbance(
    ...
    val createState: () -> State<NecromancyRituals>
)
...
private val disturbances = listOf(
    ...
    Disturbance({ it.name == "Deathshead moth" }, ::HandleDeathsheadMoth)
)

private fun checkDisturbanceState(): State<NecromancyRituals>? {
    if (System.currentTimeMillis() - lastDisturbanceTime < DISTURBANCE_COOLDOWN_MS || getXp(Skill.NECROMANCY) >= 200000000)
        return null
...
abstract class DisturbanceState(): State<NecromancyRituals>() {
    ...
    override suspend fun NecromancyRituals.checkNext() = if ((System.currentTimeMillis() - start) > timeout || handled) StartRitual else null
    abstract override suspend fun NecromancyRituals.stateLoop()
}
    ...
    override suspend fun NecromancyRituals.stateLoop() {
        findClosestNPC(SHAMBLING_HORROR)?.let { horror ->
            ...
            if (horror.interact("Sever link")) {
                ...
                delayUntil(6000) {
                    targetComponent = findClosestNPC { RITUAL_COMPONENTS.contains(it.id) && it.spotAnims.any { spotAnim -> spotAnim.id == 7977 } }
                    targetComponent != null
                }
                delay(425, 1620)
                if (targetComponent?.interact(if (targetComponent!!.name.contains("depleted")) 2 else 0) == true)
                    waitForXPDrop(Skill.NECROMANCY)
                finish()
            }
        }
...
class HandleDeathsheadMoth: DisturbanceState() {
    override suspend fun NecromancyRituals.stateLoop() {
        delay(250, 350)
        if (interactClosestNPC("Deathshead moth", "Catch")) {
            waitForXPDrop(Skill.NECROMANCY)
            delay(123, 325)
        }
        finish()
...
class HandleDefile: DisturbanceState() {
    ...
    override fun NecromancyRituals.onStateEvent(event: Event) {
        if (event is XPDrop && event.skill == Skill.NECROMANCY)
            finish()
    }
}

object StartRitual: State<NecromancyRituals>() {
    override suspend fun NecromancyRituals.checkNext() = if (localPlayer.animationId == 35520 && varps.getVarBit(RITUAL_ACTIVE_VARBIT) > 0) CheckDisturbances else null
